...
Frontend-контейнер собирает React production build и отдаёт его через nginx. При обращении к `http://localhost:5173/api/` nginx внутри frontend-контейнера проксирует запросы в backend. Путь `/media/` отдаётся из общего Docker volume с пользовательскими файлами. Путь `/admin` обслуживается React-приложением.
